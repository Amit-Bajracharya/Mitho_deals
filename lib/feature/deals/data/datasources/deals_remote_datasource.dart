...
class DealsRemoteDataSourceImpl implements DealsRemoteDataSource{
  ...
  Future<void> claimDeal(String dealId, int quantity) async {
    ...
    try {
      final response = await supabaseClient.from('deals').select('available_portions').eq('id', dealId).single();
      final current = response['available_potions'] as int;
      if(current < quantity){
        throw ServerException(message: "Not enough portion available");
      }
      await supabaseClient.from('deals').update({
  ...
  Future<List<DealModel>> getAvailableDeals() async {
    ...
    try {
      final response = await supabaseClient.from('deals').select().eq('isAvailable', true).order('pickup_start_time', ascending: true);
      return(response as List).map((json) => DealModel.fromJson(json as Map<String, dynamic>)).toList();
